...
public class GifReaderV2 {
    ...
    public GifReaderV2(Context context) {
...
//          }
    ...
    private void readImage(BufferedInputStream bis) {
        ...
        // Byte array for storing the raw image data for dynamic image generation later
        ArrayList<Byte> imageBytes = new ArrayList<>();

        try {
            byteRead = bis.read();
            minCodeSize = byteRead+1;
            currentCodeSize = minCodeSize;
            imageBytes.add((byte) byteRead);
        } catch (IOException e) {
            Log.v(TAG, e.getMessage());
        }
...
//        ArrayList<Integer> codes = new ArrayList<>();
        ...
        while(subBlockSize > 0 && !eoiReached) {
            try {
                ...
                imageBytes.add((byte) subBlockSize);
            } catch (IOException e) {
                Log.v(TAG, e.getMessage());
            }
...
//            }
        ...
        Byte[] codes1 = new Byte[imageBytes.size()];
        gifFrames.add(new GifFrame(imageBytes.toArray(codes1), activeColorTable, location_x, location_y, frameWidth, frameHeight, delayTime, disposalMethod));
...
//            break;
    ...
    public class GifFrame {
        ...
        public GifFrame(Byte[] bytes, int[] colorTable, int left, int top, int width, int height, int frameDelay, int disposalMethod) {
